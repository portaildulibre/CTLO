 ...
 abreviation
  Synonyme
   Terme_Vedette
    variante
      -->
        <xsl:when test="$parent = 'Synonyme'">
          <xsl:value-of select="'4'"/>
        </xsl:when>
        <xsl:when test="$parent = 'abreviation'">
          <xsl:choose>
            <xsl:when test="@type = 'Forme abrégée'">
              <xsl:value-of select="'2'"/>
            </xsl:when>
            <xsl:when test="@type = 'Abréviation'">
              <xsl:value-of select="'3'"/>
            </xsl:when>
            <xsl:otherwise>
	    	<xsl:value-of select="'5'"/>
              <xsl:message>Elément 'variante' de type [<xsl:value-of select="@type"/>] non reconnu.</xsl:message>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise>
